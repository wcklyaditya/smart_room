...
<body>
  ...
  <p>
    ...
    <span class="units">Percent</span>
  </p>
  
  <p>
  <i class="fa-solid fa-triangle-exclamation"></i> 
  <span class="dht-labels">Worksornot</span> 
...
void setup()
{
  ...
  server.on("/worksornot", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", String(wont).c_str());
  });
  ...
  if (currentMillis - previousMillis >= interval) 
  {
    ...
    else 
    {
      ...
      if(humi_control == 1)
      {
        ...
        {
         serverPath = humiOn;
         wont = humi_threshold[0];
        }
        if (dht_h > humi_threshold[1])
        {
         serverPath = humiOff;
         wont = humi_threshold[1];
        }
      }
